...
- **CLI Tool:** Introduced the `lazy-appwrite` CLI to bootstrap projects instantly.
  - Run `npx lazy-appwrite init` to interactively scaffold your project.
- **Auto-Configuration:** The CLI generates a production-ready `src/lib/appwrite.ts` client setup file.
- **Example Templates:** The init command now creates a `lazy-examples/` folder containing:
  - `users.ts`: A sample Schema definition.
  - `usage-guide.ts`: A code guide comparing "Lazy" usage vs "Standard" usage.
